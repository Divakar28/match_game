...
class App extends Component {
  ...
  render() {
    const {selectedTab, count} = this.state

    // Filter images based on the category of the selected tab
    const filteredImages = imagesList.filter(
      image => image.category === selectedTab,
    )

    // Calculate randomNumber here
    const randomNumber = Math.floor(Math.random() * filteredImages.length)

    return (
      <div className="c">
        <nav className="webLogo">
          <img
          ...
          <div className="scoreAndTime">
            ...
            <img
              ...
              className="timer"
            />
            <p className="count-color">{count} Sec</p>
          </div>
        </nav>
        <div>
          {/* Use randomNumber here */}
          <img
            src={filteredImages[randomNumber].imageUrl}
            alt="imageUrl"
        ...
        <ul className="tabsList">
          {tabsList.map(each => (
            <li
              key={each.tabId}
              className={`lists ${each.tabId === selectedTab ? 'active' : ''}`}
              onClick={() => this.setState({selectedTab: each.tabId})}
            >
              {each.displayText}
            </li>
          ))}
